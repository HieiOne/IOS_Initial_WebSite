        <!-- HEADER -->
        <div class="header">
            <!-- TOP-BAR -->
            <div class="top-bar">
                <div class="game-title">
                    <a href="index.php"><h1>IOSOCCER</h1></a>
                </div>

                <div class="search-bar">
                    <form action="#" method="GET">
                        <input class="search" type="text" name="search" placeholder="Search..">
                    </form>
                </div>
                <div class="buttons">
                    <div class="dropdown">
                        <a><img class="dropbtn" src="./img/bars.png"></img></a>
                        <div class="dropdown-content">
                            <a class="button" id="button0" href="index.php">HOME</a>
                            <a class="button" id="button1" href="news.php">NEWS</a>
                            <a class="button" id="button2" href="leaderboard.php">LEADERBOARD</a>
                            <a class="button" id="button3" href="#">ABOUT</a>
                            <a class="button" id="button4" href="#">CONTACT</a>
                            <a class="button" id="button5" href="#">LOGIN</a>
                        </div>
                    </div>
                    <a class="button" id="button6" href="index.php" id="button-home"><div><h1>HOME</h1></div></a>
                    <a class="button" id="button7" href="news.php" id="button-news"><div><h1>NEWS</h1></div></a>
                    <a class="button" id="button8" href="leaderboard.php" id="button-league"><div><h1>LEADERBOARD</h1></div></a>
                    <a class="button" id="button9" href="#" id="button-about"><div><h1>ABOUT</h1></div></a>
                    <a class="button" id="button10" href="#" id="button-contact"><div><h1>CONTACT</h1></div></a>
                    <a class="button" id="button11" href="#" id="button-login"><div><h1 class="login">LOGIN</h1></div></a>
                </div>
            </div>
        </div>